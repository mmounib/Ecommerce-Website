...
const navLinks = allLinks.map(navLink => {
    return (
        <li className="nav_item my-8" key={navLink.id}>
            <a href={navLink.to} className='nav_link p-3 text-base font-bold rounded-md hover:bg-primary-color'>{navLink.value}</a>
        </li>
    )
})
        ...
        <div className='flex flex-col flex-grow justify-between'>
          ...
          <footer className='flex justify-between'>
            <div className='flex justify-center items-center h-10 border-2 border-gray-300 text-gray-300'>
              <div className='w-10 cursor-pointer' onClick={() => setNumber(prev => prev - 1)}>-</div>
              <div className='w-10 border-x-2 text-secondary-color'>{number}</div>
              <div className='w-10 cursor-pointer' onClick={() => setNumber(prev => prev + 1)}>+</div>
            </div>
            <img src={trash} alt='' />
          </footer>
    ...
    <header className='flex justify-between items-center text-xl px-8 lg:px-16 xl:px-24 py-7 border-b-2 border-l-gray-400'>
      ...
      <div className='flex gap-4'>
        ...
        <HiMenu className="menu md:hidden w-6 h-6 cursor-pointer" onClick={() => setOn(prev => !prev)}/>
          {
            On &&
            <nav id="mobile-nav" className='nav fixed top-0 right-0 bg-secondary-color text-primary-color w-11/12 h-full p-12 z-50 rounded-bl-full'>
      ...
      {showList && <div className='absolute top-0 right-0 h-screen w-screen z-10 bg-secondary-color bg-opacity-30'>
          <div className='list flex flex-col items-center gap-12 ml-auto w-[600px] h-screen bg-primary-color px-12'>
            ...
            <Link to='/payment' className='w-full py-4 rounded-lg mb-12 bg-secondary-color text-primary-color' onClick={() => setShowList(false)}>Check Out - {totalPrice}.00MAD</Link>
          </div>
      </div>}
    </header>
);
